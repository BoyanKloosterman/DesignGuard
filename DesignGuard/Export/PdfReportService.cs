...
public sealed class PdfReportService
{
        ...
        IReadOnlyList<PdfC4MermaidBandImage>? c4MermaidBands)
    {
        ...
        var chapters = BuildChapterList(diagramPng, hasC4Mermaid);
        ...
        return Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                page.Header().SkipOnce().Column(hdr =>
                {
                    hdr.Item().BorderBottom(1).BorderColor(PdfPalette.Border).PaddingBottom(8).Row(row =>
                    {
                    ...
                    hdr.Item().PaddingTop(3).AlignRight().Text("Hoofdstuknummers staan bij elke nieuwe pagina onder de inhoudsopgave.")
                        .FontSize(7.5f).FontColor(PdfPalette.Muted);
                });
                ...
                page.Content().Column(col =>
                {
                    ...
                    for (var i = 0; i < chapters.Count; i++)
                        AddTocLine(col, chapters[i].Id, $"{i + 1}. {chapters[i].TocTitle}");
                    ...
                    for (var i = 0; i < chapters.Count; i++)
                    {
                        ...
                        var ch = chapters[i];
                        col.Item().Section(ch.Id).Column(s =>
                        {
                            s.Spacing(10);
                            s.Item().EnsureSpace(72);
                            s.Item().Element(ChapterTitle(i + 1, ch.HeadingTitle));
                            ch.BuildBody(s, project, threats, requirements, diagramPng, c4MermaidBands);
                        });
                    }
                });
            });
    ...
    private sealed record PdfChapterDef(
        ...
        string TocTitle,
        string HeadingTitle,
        Action<ColumnDescriptor, ProjectModel, IReadOnlyList<ThreatModel>, IReadOnlyList<RequirementModel>, byte[]?, IReadOnlyList<PdfC4MermaidBandImage>?> BuildBody);

    private List<PdfChapterDef> BuildChapterList(byte[]? diagramPng, bool hasC4Mermaid)
    {
        var list = new List<PdfChapterDef>
        {
            new(Sec.Summary, "Executive summary", "Executive summary", BuildSummaryBody),
            new(Sec.SystemContext, "Systeemcontext", "Systeemcontext", BuildSystemContextBody)
        };

        if (diagramPng is { Length: > 0 })
            list.Add(new(Sec.ArchDiagram, "Architectuurdiagram (Mermaid)", "Architectuurdiagram", BuildArchBody));

        list.Add(new(Sec.TrustBoundaries, "Trust boundaries", "Trust boundaries", BuildTrustBody));
        ...
        list.Add(new(Sec.Controls, "Controls", "Controls", BuildControlsBody));
        list.Add(new(Sec.Decisions, "Beslissingen en aannames", "Beslissingen en aannames", BuildDecisionsBody));
        list.Add(new(Sec.Packs, "Knowledge packs (actief geladen)", "Knowledge packs (actief geladen)", BuildPacksBody));
        list.Add(new(Sec.OpenIssues, "Open issues", "Open issues", BuildOpenIssuesBody));

        return list;
    }

    private static void BuildSummaryBody(
        ColumnDescriptor s,
        ProjectModel project,
        IReadOnlyList<ThreatModel> threats,
        IReadOnlyList<RequirementModel> requirements,
        byte[]? _,
        IReadOnlyList<PdfC4MermaidBandImage>? __)
    {
        s.Item().Text(
    ...
    private static void BuildSystemContextBody(
        ColumnDescriptor s,
        ProjectModel project,
        IReadOnlyList<ThreatModel> _,
    ...
    private static void BuildArchBody(
        ColumnDescriptor s,
        ProjectModel _,
        IReadOnlyList<ThreatModel> __,
        IReadOnlyList<RequirementModel> ___,
        byte[]? diagramPng,
        IReadOnlyList<PdfC4MermaidBandImage>? ____)
    {
        if (diagramPng is not { Length: > 0 }) return;
        s.Item().Text(
    ...
    private static void BuildTrustBody(
        ColumnDescriptor s,
        ProjectModel project,
        IReadOnlyList<ThreatModel> _,
        IReadOnlyList<RequirementModel> __,
    ...
    private static void BuildC4VisualBody(
        ColumnDescriptor s,
        ProjectModel _,
        IReadOnlyList<ThreatModel> __,
        IReadOnlyList<RequirementModel> ___,
        ...
        IReadOnlyList<PdfC4MermaidBandImage>? c4MermaidBands)
    {
        s.Item().Text(
            ...
            .FontSize(9.5f).LineHeight(1.3f).FontColor(PdfPalette.Muted);
        if (c4MermaidBands == null) return;
        foreach (var band in c4MermaidBands)
        {
            if (band.Png is not { Length: > 0 }) continue;
            s.Item().Element(SubsectionTitle(band.Caption));
            s.Item().PaddingTop(4).MinHeight(220).Image(band.Png).FitArea();
        }
    }

    private static void BuildC4ScopeBody(
        ColumnDescriptor s,
        ProjectModel project,
        IReadOnlyList<ThreatModel> threats,
        IReadOnlyList<RequirementModel> _,
        byte[]? __,
        IReadOnlyList<PdfC4MermaidBandImage>? ___)
    {
        s.Item().Text(
        ...
        s.Item().Element(SubsectionTitle("Zoomniveaus (kort)"));
        foreach (var lvl in new[] { C4Level.Context, C4Level.Container, C4Level.Component, C4Level.Code })
        {
        ...
        foreach (var lvl in new[] { C4Level.Context, C4Level.Container, C4Level.Component, C4Level.Code })
        {
            ...
            s.Item().Element(SubsectionTitle($"{C4LevelFormatting.ShortLabel(lvl)} — elementen"));
    ...
    private static void BuildRequirementsBody(
        ColumnDescriptor s,
        ProjectModel _,
        IReadOnlyList<ThreatModel> __,
        IReadOnlyList<RequirementModel> requirements,
    ...
    private static void BuildTraceBody(
        ColumnDescriptor s,
        ProjectModel _,
        IReadOnlyList<ThreatModel> threats,
        IReadOnlyList<RequirementModel> requirements,
        ...
        IReadOnlyList<PdfC4MermaidBandImage>? ___)
    {
        ...
        s.Item().Element(SubsectionTitle("Dreigingen"));
        foreach (var t in threats.OrderBy(x => x.StrideCategory).ThenBy(x => x.Title).Take(PdfTraceabilityTriggerMax))
        {
            ...
            s.Item().PaddingLeft(8).Text($"{t.Title}: {keys}").FontSize(9.5f);
        }
        ...
        s.Item().Element(SubsectionTitle("Security-eisen"));
        foreach (var r in requirements.OrderBy(x => x.Category).ThenBy(x => x.Title).Take(PdfTraceabilityTriggerMax))
        {
            ...
            s.Item().PaddingLeft(8).Text($"{r.Title}: {keys}").FontSize(9.5f);
        }
    ...
    private static void BuildControlsBody(
        ColumnDescriptor s,
        ProjectModel project,
        IReadOnlyList<ThreatModel> _,
        IReadOnlyList<RequirementModel> __,
    ...
    private static void BuildDecisionsBody(
        ColumnDescriptor s,
        ProjectModel project,
        IReadOnlyList<ThreatModel> _,
        IReadOnlyList<RequirementModel> __,
    ...
    private void BuildPacksBody(
        ColumnDescriptor s,
        ProjectModel _,
        IReadOnlyList<ThreatModel> __,
        IReadOnlyList<RequirementModel> ___,
        byte[]? ____,
        IReadOnlyList<PdfC4MermaidBandImage>? _____)
    {
        foreach (var p in _packs.LoadedPacks)
            s.Item().Text($"• {p.Dto.DisplayLabel} — v{p.Dto.VersionLabel} — {p.Dto.SourceName}");
    }

    private static void BuildOpenIssuesBody(
        ColumnDescriptor s,
        ProjectModel project,
        IReadOnlyList<ThreatModel> _,
        IReadOnlyList<RequirementModel> __,
    ...
    private static Action<IContainer> ChapterTitle(int chapterNumber, string title) => container =>
    {
    ...
    private static Action<IContainer> SubsectionTitle(string title) => container =>
    {
